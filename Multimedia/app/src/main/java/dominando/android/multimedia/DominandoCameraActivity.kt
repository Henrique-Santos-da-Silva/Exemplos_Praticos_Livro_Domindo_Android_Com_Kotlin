...
class DominandoCameraActivity : Activity(), CoroutineScope {
    ...
    private fun requestPermissions() {
        ActivityCompat.requestPermissions(this, arrayOf(Manifest.permission.CAMERA, Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.RECORD_AUDIO), REQUEST_PERMISSIONS)
    }
    ...
    private val pictureCallback = Camera.PictureCallback {data, camera ->
        outputUri?.let { uri ->
            try {
                val fileDescription = contentResolver.openFileDescriptor(uri, "rw")?.fileDescriptor
                val fos = FileOutputStream(fileDescription)
                fos.write(data)
                fos.close()
                inPreviewMode = true
                btnCapture.text = "OK"
    ...
    private fun prepareRecording(): Boolean {
        ...
        outputUri?.let { uri ->
            try {
                val fileDescriptor = contentResolver.openFileDescriptor(uri, "rw")?.fileDescriptor
