...
class CameraVideoFragment: MultimediaFragment(), ViewTreeObserver.OnGlobalLayoutListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (videoUri == null) {
            val videoPath = MediaUtils.getLastMediaPath(requireActivity(), MediaUtils.MediaType.MEDIA_VIDEO)
            if (videoPath != null) {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val layout = inflater.inflate(R.layout.fragment_camera_video, container, false)
    ...
    private val onVideoActivityResult = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            ...
            activity?.let {
                try {
                    ...
                    if (captureVideoIntent.resolveActivity(it.packageManager) != null) {
                        onVideoActivityResult.launch(Intent(captureVideoIntent))
                    }
        ...
        activity?.let {
            videoUri?.let { uri ->
                if (hasPermission()) {
                    ...
                    MediaUtils.saveLastMediaPath(it, MediaUtils.MediaType.MEDIA_VIDEO, uri.toString())
                } else {
