...
//    var instantTaskExecutorRule = InstantTaskExecutorRule()
    ...
    fun before_each_test() {
        val db = Room.inMemoryDatabaseBuilder(InstrumentationRegistry.getInstrumentation().context, HotelDatabase::class.java).build()
    ...
    @Test
    fun insert_hotel_must_change_id_and_start() {
    ...
    @Test
    fun insert_the_same_hotel_must_update_the_record() {
        repo.save(dummyHotel)
        val id = dummyHotel.id
        assertNotEquals(id, 0)
        val hotel1 = LiveDataTestUtil.getValues(repo.hotelById(id))
        repo.save(dummyHotel)
        val hotel2 = LiveDataTestUtil.getValues(repo.hotelById(id))
    ...
    fun load_all_inserted_hotels() {
        ...
        val returnedList = LiveDataTestUtil.getValues(repo.search("%"))
    ...
    fun remove_hotel() {
        ...
        val returnedHotel = LiveDataTestUtil.getValues(repo.hotelById(dummyHotel.id))
