...
class FavoritesWidget : AppWidgetProvider() {
    ...
    private fun initButtons(context: Context, appWidgetId: Int, views: RemoteViews) {
        views.setOnClickPendingIntent(R.id.btnNext, actionPendingIntent(context, ACTION_NEXT, appWidgetId))
        views.setOnClickPendingIntent(R.id.btnPrevious, actionPendingIntent(context, ACTION_PREVIOUS, appWidgetId))
        views.setOnClickPendingIntent(R.id.txtSite, actionPendingIntent(context, ACTION_SITE, appWidgetId))
    }

    private fun actionPendingIntent(ctx: Context, action: String, appWidgetId: Int): PendingIntent {
            ...
            return PendingIntent.getBroadcast(ctx, appWidgetId+action.hashCode(), actionIntent, PendingIntent.FLAG_UPDATE_CURRENT)
    }
