...
class FavoritesReceiver: BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent?) {
        if (intent != null) {
            ...
            if (action != null) {
                when(action) {
                    FavoritesWidget.ACTION_NEXT, FavoritesWidget.ACTION_PREVIOUS -> {
                    ...
                    FavoritesWidget.ACTION_DELETE -> {
                        val deleteWidgets = intent.getIntArrayExtra(AppWidgetManager.EXTRA_APPWIDGET_IDS) as IntArray
                        for (id in deleteWidgets) {
                            remove(context, id)
                        }
                    }
