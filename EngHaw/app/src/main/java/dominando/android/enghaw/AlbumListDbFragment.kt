...
class AlbumListDbFragment: AlbumListBaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        rvAlbums.layoutManager = if (resources.configuration.orientation == Configuration.ORIENTATION_PORTRAIT) {
            LinearLayoutManager(requireActivity())
        } else {
            GridLayoutManager(requireActivity(), 2)
        }
        rvAlbums.addItemDecoration(DividerItemDecoration(context, LinearLayoutManager.VERTICAL))
