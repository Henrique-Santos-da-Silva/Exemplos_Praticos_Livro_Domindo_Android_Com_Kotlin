...
class FbRepository {
    ...
    fun loadBooks(): LiveData<List<Book>> {
        return object : LiveData<List<Book>>() {
            override fun onActive() {
                super.onActive()
                firestore.collection(BOOKS_KEY).whereEqualTo(USER_ID_KEY, currentUser?.uid)
    ...
    fun remove(book: Book): LiveData<Boolean> {
        return object : LiveData<Boolean>() {
            override fun onActive() {
                super.onActive()
                val db = firestore
                db.collection(BOOKS_KEY).document(book.id).delete()
    ...
    private fun uploadPhoto(book: Book): Task<Uri> {
        ...
        val storeRef = storageRef.child(book.id)
        return storeRef.putFile(Uri.parse(book.coverUrl)).continueWithTask { uploadTask ->
